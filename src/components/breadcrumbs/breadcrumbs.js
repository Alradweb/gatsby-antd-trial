...
const Breadcrumbs = props => {
    ...
    const extraBreadcrumbItems = pathSnippets.map((snippet, index) => {
        ...
        const newAlias = props.alias
            ? props.alias[0].toUpperCase() + props.alias.slice(1)
            : ""
        const title =
            pathsLength === index + 1 ? props.title : newAlias || snippet
        return (
            <Breadcrumb.Item key={url}>
                <Link to={url} aria-label={`Переход на страницу ${title}`}>
                    {title}
                </Link>
            </Breadcrumb.Item>
        )
    ...
    return (
        <Breadcrumb className={styles.breadcrumbs} separator={"»"}>
            <Breadcrumb.Item>
                <Link to={"/"} aria-label={`Переход на домашнюю страницу`}>
