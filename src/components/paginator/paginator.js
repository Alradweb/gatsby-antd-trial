...
const Paginator = (props) => {
  ...
  function renderItems(i, type) {
    ...
    if (type === "page") {
      return i === 1 ? (
        <Link className={i === currentPage ? styles.activeHref : ""} to={`/${pageName}/`}>{i}</Link>) : (
        <Link className={i === currentPage ? styles.activeHref : ""} to={`/${pageName}/${i}`}>{i}</Link>)
    }
    if (type === "next") {
      return isLast ? <Icon type="double-right" style={{ color: "rgba(0, 0, 0, 0.65)", fontSize: "15px" }}/>
        : <Link to={`/${pageName}/${nextPage}`} rel="next" title={"Следующая страница"}>
          <Icon type="double-right" style={{ color: "tomato", fontSize: "15px" }}/>
        </Link>
    }
    if (type === "prev") {
      return isFirst ? <Icon type="double-left" style={{ color: "rgba(0, 0, 0, 0.65)", fontSize: "15px" }}/>
      : <Link to={`/${pageName}/${prevPage}`} rel="prev" title={"Предыдущая страница"}>
